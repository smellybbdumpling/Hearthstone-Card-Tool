...
后端默认允许 `http://localhost:5173` 和 `http://127.0.0.1:5173` 访问，并放行跨域 `OPTIONS` 预检请求。预检请求不改变业务接口的响应结构。
...
前端在访问需要登录的接口时，如果收到 `401` 或 `403`，会删除本地 `hs_token`、`hs_user` 并跳转到 `/login`。访问受保护路由时，前端会通过 `GET /api/auth/me` 刷新当前用户信息。
